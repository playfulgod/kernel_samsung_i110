...
void qt_Noise_Suppression_Config_Init(void)
{
	...
	if (info_block->info_id.family_id == 0x80) {
		...
		noise_suppression_config_e.cfg = 4;// 1
		noise_suppression_config_e.calcfg = 64;// 2
#ifdef CONFIG_TIKAL_USCC		
		noise_suppression_config_e.basefreq = 29;// 3
#else
		...
		noise_suppression_config_e_TA.cfg = 4;// 1
		noise_suppression_config_e_TA.calcfg = 80;// 2
#ifdef CONFIG_TIKAL_USCC				
		noise_suppression_config_e_TA.basefreq = 29;// 3
#else
		...
		noise_suppression_config_e_TA.tchthr = 70;//35
...
void qt_CTE_Config_Init(void)
{
	if (info_block->info_id.family_id == 0x80) {
		...
		cte_config_e.mode = 3;
#ifdef CONFIG_TIKAL_USCC
		cte_config_e.idlesyncsperx = 24;
		cte_config_e.actvsyncsperx = 40;
#else
...
#ifdef CONFIG_TIKAL_USCC		
		val=noise_suppression_config_e_TA.blen;
		write_mem(object_address+34, size_one, &val);
#endif		
	}
...
static ssize_t set_noise_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (info_block->info_id.family_id == 0x80) {
		...
		if (cmd_no == 0) {
			dprintk("[%s] CMD 0 , noise_suppression_config_e_TA.ctrl = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.ctrl = config_value;
		} else if (cmd_no == 1) {
			dprintk("[%s] CMD 1 , noise_suppression_config_e_TA.cfg = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.cfg = config_value;
		} else if (cmd_no == 2) {
			dprintk("[%s] CMD 2 , noise_suppression_config_e_TA.calcfg = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.calcfg = config_value;
		} else if (cmd_no == 3) {
			dprintk("[%s] CMD 3 , noise_suppression_config_e_TA.basefreq = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.basefreq = config_value;
		} else if (cmd_no == 4) {
			dprintk("[%s] CMD 4 , noise_suppression_config_e_TA.mffreq[0] = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.mffreq[0] = config_value;
		} else if (cmd_no == 5) {
			dprintk("[%s] CMD 5 , noise_suppression_config_e_TA.mffreq[1] = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.mffreq[1] = config_value;
		} else if (cmd_no == 6) {
			dprintk("[%s] CMD 6 , noise_suppression_config_e_TA.gcactvinvldadcs = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.gcactvinvldadcs = config_value;
		} else if (cmd_no == 7) {
			dprintk("[%s] CMD 7 , noise_suppression_config_e_TA.gcidlenivldadcs = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.gcidlenivldadcs = config_value;
		} else if (cmd_no == 8) {
			dprintk("[%s] CMD 8 , noise_suppression_config_e_TA.gcmaxadcsperx = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.gcmaxadcsperx = config_value;
		} else if (cmd_no == 9) {
			dprintk("[%s] CMD 9 , noise_suppression_config_e_TA.gclimitmin = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.gclimitmin = config_value;
		} else if (cmd_no == 10) {
			dprintk("[%s] CMD 10 , noise_suppression_config_e_TA.gclimitmax = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.gclimitmax = config_value;
		} else if (cmd_no == 11) {
			dprintk("[%s] CMD 11 , noise_suppression_config_e_TA.gccountmintgt = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.gccountmintgt = config_value;
		} else if (cmd_no == 12) {
			dprintk("[%s] CMD 12 , noise_suppression_config_e_TA.mfinvlddiffthr = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.mfinvlddiffthr = config_value;
		} else if (cmd_no == 13) {
			dprintk("[%s] CMD 13 , noise_suppression_config_e_TA.mfincadcspxthr = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.mfincadcspxthr = config_value;
		} else if (cmd_no == 14) {
			dprintk("[%s] CMD 14 , noise_suppression_config_e_TA.mferrorthr = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.mferrorthr = config_value;
		} else if (cmd_no == 15) {
			dprintk("[%s] CMD 15 , noise_suppression_config_e_TA.selfreqmax = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.selfreqmax = config_value;
		} else if (cmd_no == 16) {
			dprintk("[%s] CMD 16 , noise_suppression_config_e_TA.blen = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.blen = config_value;
		} else if (cmd_no == 17) {
			dprintk("[%s] CMD 17 , noise_suppression_config_e_TA.tchthr = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.tchthr = config_value;
		} else if (cmd_no == 18) {
			dprintk("[%s] CMD 18 , noise_suppression_config_e_TA.tchdi = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.tchdi = config_value;
		} else if (cmd_no == 19) {
			dprintk("[%s] CMD 19 , noise_suppression_config_e_TA.movhysti = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.movhysti = config_value;
		} else if (cmd_no == 20) {
			dprintk("[%s] CMD 20 , noise_suppression_config_e_TA.movhystn = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.movhystn = config_value;
		} else if (cmd_no == 21) {
			dprintk("[%s] CMD 21 , noise_suppression_config_e_TA.movfilter = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.movfilter = config_value;
		} else if (cmd_no == 22) {
			dprintk("[%s] CMD 22 , noise_suppression_config_e_TA.numtouch = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.numtouch = config_value;
		} else if (cmd_no == 23) {
			dprintk("[%s] CMD 23 , noise_suppression_config_e_TA.mrghyst = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.mrghyst = config_value;
		} else if (cmd_no == 24) {
			dprintk("[%s] CMD 24 , noise_suppression_config_e_TA.mrgthr = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.mrgthr = config_value;
		} else if (cmd_no == 25) {
			dprintk("[%s] CMD 25 , noise_suppression_config_e_TA.xloclip = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.xloclip = config_value;
		} else if (cmd_no == 26) {
			dprintk("[%s] CMD 26 , noise_suppression_config_e_TA.xhiclip = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.xhiclip = config_value;
		} else if (cmd_no == 27) {
			dprintk("[%s] CMD 27 , noise_suppression_config_e_TA.yloclip = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.yloclip = config_value;
		} else if (cmd_no == 28) {
			dprintk("[%s] CMD 28 , noise_suppression_config_e_TA.yhiclip = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.yhiclip = config_value;
		} else if (cmd_no == 29) {
			dprintk("[%s] CMD 29 , noise_suppression_config_e_TA.xedgectrl = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.xedgectrl = config_value;
		} else if (cmd_no == 30) {
			dprintk("[%s] CMD 30 , noise_suppression_config_e_TA.xedgedist = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.xedgedist = config_value;
		} else if (cmd_no == 31) {
			dprintk("[%s] CMD 31 , noise_suppression_config_e_TA.yedgectrl = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.yedgectrl = config_value;
		} else if (cmd_no == 32) {
			dprintk("[%s] CMD 32 , noise_suppression_config_e_TA.yedgedist = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.yedgedist = config_value;
		} else if (cmd_no == 33) {
			dprintk("[%s] CMD 33 , noise_suppression_config_e_TA.jumplimit = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.jumplimit = config_value;
		} else if (cmd_no == 34) {
			dprintk("[%s] CMD 34 , noise_suppression_config_e_TA.tchhyst = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.tchhyst = config_value;
		} else if (cmd_no == 35) {
			dprintk("[%s] CMD 35 , noise_suppression_config_e_TA.nexttchdi = %d\n", __func__, config_value );
			noise_suppression_config_e_TA.nexttchdi = config_value;
		} else
			dprintk("[%s] unknown CMD\n", __func__);
