...
#define KEYPAD_COL_OFFSET		2
#else
#define KEYPAD_COL_OFFSET       0
#endif
...
static int keypad_scan(void)
{
	...
	for (col=KEYPAD_COL_OFFSET; col < (KEYPAD_COLUMNS + KEYPAD_COL_OFFSET); col++) {
#if defined(CONFIG_MACH_CHIEF) && !defined(CONFIG_TIKAL_MPCS)   
...
#endif
#endif
		i = (col-KEYPAD_COL_OFFSET) * KEYPAD_ROWS;
...
static void s3c_keypad_starttimer(struct s3c_keypad * s3c_keypad)
{
	...
	if( s3c_keypad->keypad_timer_on ) {
		...
		init_timer(timer);
#if defined(CONFIG_MACH_CHIEF) && !defined(CONFIG_TIKAL_MPCS)
		/*When any key is pressed and hold while playing MP3, MP3 player does not get enough time and so we 
		hear breaks in the playback. So changed the keypad scanning interval from 30ms to 40ms*/
		timer->expires		= jiffies + (4*HZ/100);	
#else	
