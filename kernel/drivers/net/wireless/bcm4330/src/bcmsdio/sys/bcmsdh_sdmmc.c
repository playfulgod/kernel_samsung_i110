...
static int err_print_w = 0; //satish
static int err_print_r = 0; //satish
extern SDIOH_API_RC
...
//		                        rw ? "Write" : "Read", func, regaddr, *byte, err_ret));
		...
		if (++err_print_w <= 30 && rw)		//110804
			sd_err(("bcmsdh_sdmmc: Failed to %s byte F%d:@0x%05x=%02x, Err: %d, num: %d\n",
		                        rw ? "Write" : "Read", func, regaddr, *byte, err_ret, err_print_w));
		if (++err_print_r <= 30 && !rw)		//110804
			sd_err(("bcmsdh_sdmmc: Failed to %s byte F%d:@0x%05x=%02x, Err: %d, num: %d\n",
		                        rw ? "Write" : "Read", func, regaddr, *byte, err_ret, err_print_r));
	}
