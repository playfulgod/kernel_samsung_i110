...
dhdsdio_htclk(dhd_bus_t *bus, bool on, bool pendok)
{
	...
	if (on) {
		...
		if (err) {
			if (++err_cnt_1 <= 10) {
				DHD_ERROR(("%s: HT Avail request error: %d, num %d\n", __FUNCTION__, err, err_cnt_1));
				stop_printing=1;
			}
			return BCME_ERROR;
		}
		...
		if (err) {
			if (++err_cnt_2 <= 10) {
				DHD_ERROR(("%s: HT Avail read error: %d, num %d\n", __FUNCTION__, err, err_cnt_2));
				stop_printing=1;
			}
			return BCME_ERROR;
		}
		...
		if (!SBSDIO_CLKAV(clkctl, bus->alp_only) && pendok) {
			...
			if (err) {
				if (++err_cnt_3 <= 10) {
					DHD_ERROR(("%s: Devctl access error setting CA: %d, num %d\n",
		...
		if (!SBSDIO_CLKAV(clkctl, bus->alp_only)) {
			if (++err_cnt_5 <= 10) {
				DHD_ERROR(("%s: HT Avail timeout (%d): clkctl 0x%02x, num %d\n",
...
dhdsdio_bussleep(dhd_bus_t *bus, bool sleep)
{
	...
	if (sleep) {
		...
		if(stop_printing==0)		
			DHD_ERROR(("%s, %d, CLK_AVAIL requested\n", __FUNCTION__, __LINE__));		//110804
...
dhdsdio_dpc(dhd_bus_t *bus)
{
	...
	/* Make sure backplane clock is on */
	if(stop_printing==0)		
		DHD_ERROR(("%s, %d, CLK_AVAIL requested, pending TRUE\n", __FUNCTION__, __LINE__));		//110804
	dhdsdio_clkctl(bus, CLK_AVAIL, TRUE);
	if (bus->clkstate != CLK_AVAIL)
		goto clkwait;
