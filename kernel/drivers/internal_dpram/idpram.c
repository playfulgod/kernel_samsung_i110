...
void send_interrupt_to_phone(u16 irq_mask)
{
	...
	if(temp != 0){
		...
		WRITE_TO_DPRAM(DPRAM_PDA2PHONE_INTERRUPT_ADDRESS,
			&clear, DPRAM_INTERRUPT_PORT_SIZE);
		mdelay(1);
		//udelay(500);
	}
...
static int dpram_read_raw(dpram_device_t *device, const u16 non_cmd)
{
	...
	if (head != tail) {
		...
		if(len & 0x01) read_offset++;	// Debasish. To skip odd byte of data.
...
void non_command_handler(u16 non_cmd)
{
	...
	if (non_cmd & INT_MASK_RES_ACK_R) {
		...
		MULTIPDP_LOG_ERR("[DPRAM] receive res ack from phone.\n");
	}
}
...
static int pdp_mux_tty(struct pdp_info *dev, const void *data, size_t len)
{
	...
	while (count) {
		...
		if (elapsedtime_tv.tv_usec > 100000) {
